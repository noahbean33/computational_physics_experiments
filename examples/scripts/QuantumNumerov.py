...
# QuantumNumerov.py: Solve quantum bound state via Numerov algorithm
# hbarc* omega=hbarc*sqrt(k/m)=19.733,  r mc**2=940 MeV, k=9.4
# E =(N+1/2)hbarc*omega = (N+1/2)19.733, N=0,2,4, change if N odd

from numpy import *
import numpy as np, matplotlib.pyplot as plt

n = 1000; m = 2;  imax = 100;  Xleft0 = -10; Xright0 = 10; h  = 0.02
amin= 81.; amax = 92.;  e = amin;  de  = 0.01;   eps=  1e-4; im = 500 
nl = im + 2;  nr = n - im + 2; xmax = 5.0                    
print("nl, nr",nl, nr)
print(h)
...
def V(x):                          # Potential harmonic oscillator
    v = 4.7*x*x                  
    return v
def setk2(e):                      # Set k2L=(sqrt(e-V))^2, k2R 
    for i in range(0,n):         
       xLeft = Xleft0 + i*h
       xr = Xright0 - i*h
       fact=0.04829                # 2m*c**2/hbarc**2
       k2L[i] = fact*(e-V(xLeft))
       k2R[i] = fact*(e-V(xr))
def Numerov (n,h,k2,u,e):            
    setk2(e)
    b=(h**2)/12.0                  # L & R wave functions
    for i in range( 1,n):  
