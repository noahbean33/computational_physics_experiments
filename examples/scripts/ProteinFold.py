...
size  = 8;  size2 = size*2;  nex = 0
M = []; DD = []              # Arrays for polymer & grid

graph1 = display(width=Maxx, height=Maxy,title='Protein Folding',
                  range=ran)
        ...
        if cl==1: pass   # if white
           ...
           if n < length+1:
              ...
              if imin >= 0:
                  ...
                  energy = energy + e
                  if e==1:    # Plot yellow dot at neighbour
                       xol = 4*(i-0.5)-size2
                       yol = -4*j+size2
              ...
              if ima<=size-1:      # Check neighborr i+1,j
                  ...
                  energy = energy+e
                  if e == 1:     # Plot yellow dot at neighbor
                       xol = 4*(i+0.5)-size2  
                       yol = -4*j+size2     
                       points(pos=(xol,yol),color=color.yellow, size=6)
              iss = i
...
grid()
length = 0
while 1:                              # Adjust for desired number of walks
    ...
    while (i>0 and i<size-1 and j>0 and j<size-1 and (grid[i+1,j] == 0
        ...
        if r < 0.25 :                   # Probability 25%
            if grid[i+1,j]==0:  i += 1  # Step right if empty      
        elif 0.25 < r and r < 0.5:     # Step left
            if grid[i-1,j] == 0: i -= 1    
        ...
        if grid[i,j] == 0:
           ...
           xp = 4*i-size2       
           yp = -4*j+size2                   
           curve(pos=[(xol,yol),(xp,yp)])  # Connect last to new position
           M = M + [points(pos=(xp,yp), color=col,size=6)]
           xol = xp                          # Start new line
           yol = yp                          
        while (j == (size-1) and i != 0 and i != (size-1)):   # Bottom row
            r1 = random.random()
            if r1 < 0.2:                      # Probability 20% move left
                if grid[i-1,j] == 0: i -= 1
        ...
        while (i==0 and j !=0 and j !=(size-1)): # First column
            ...
            if j==(size-1) or j==0 or (grid[0,j+1]!=0 and grid[0,j-1]!=0):
                break
        while (i==(size-1) and j !=0 and j !=(size-1)): # Last column
            r1 = random.random()
            if r1 < 0.2:
                if grid[i,j-1] == 0: j -= 1
            elif r1 > 0.2 and r1 < 0.4:
    ...
    label(pos=(10,18,0), text='Click for new walk',color=color.red, display=graph1)
    pts2.text = '%4s' %length       
    label(pos=(5,-18,0), text='Energy',box=0)
    evalue=label(pos=(10, -18), box=0) # Energy
    evalue.text = '%4s' %findenergy(length,DD)    # Walk length walk
