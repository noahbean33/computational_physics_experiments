...
y = zeros(N+1, float); Ycomplex = zeros(N, complex)   # Declare arrays
...
def DFT(Ycomplex):                                      # DFT
    for n in range(0, N):              
      ...
      for  k in range(0, N):                              
          zexpo = complex(0, twopi*k*n/N)               # Complex exp
          zsum += y[k]*exp(-zexpo)           
      Ycomplex[n] = zsum * sq2pi                                      
      if Ycomplex[n].imag != 0: TransformCurve.plot(pos=(n,Ycomplex[n].imag)) 
