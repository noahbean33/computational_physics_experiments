...
Ex = zeros((xmax,ts),float); Ey = zeros((xmax,ts),float); Hx = zeros((xmax,ts),float)
...
plate = box(pos=(-100,0,0),height=2*zmax,width=2*ymax,length=0.5*xmax,
          color=(1.0,0.6,0.0),opacity=0.4)
...
def  InitExHy():
    ...
    Ex[101:202,0]  =  0.1*cos(-2*pi*kk/100.0-0.005*pi*(kk-101)) # pi/2 phase
...
while end < 5:
    ...
    Hyy[0,1] = Hyy[0,0] + beta*(Exx[xmax-2,0] - Exx[1,0])    # Periodic BC, 0=100
    ...
    Hy[0,1] = Hyy[0,0] + beta*(Exx[xmax-2,0] - Exx[1,0])    # Periodic BC 0=100
